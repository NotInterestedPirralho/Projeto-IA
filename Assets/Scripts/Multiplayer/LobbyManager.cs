...
using TMPro; // Certifique-se de que está a usar TextMeshPro
...
public class LobbyManager : MonoBehaviourPunCallbacks
{
    ...
    private void CheckStartConditions()
    {
        ...
        {
            // Condição 1: Sala Cheia (Começa em 5 segundos)
            // Se não estiver contando, ou se estiver contando com o timer de 90s, resetar para 5s
        ...
        {
            // Condição 2: Segundo jogador entrou (Começa em 90 segundos)
            // APENAS se não estiver JÁ contando regressiva com 90s (para não resetar)
            // e para não interromper um timer de 5s se a sala esvaziou de 4 para 3.
            if (!isCountingDown && countdownDuration != WAIT_TIME_FULL_ROOM)
            {
        ...
        {
            // Condição 3: Menos de 2 jogadores (Para a contagem)
            StopCountdown();
        }
    }
    ...
    private void StartGame()
    {
        ...
        // 1. Define a propriedade da sala.
        // O callback OnRoomPropertiesUpdate() tratará do spawn para TODOS.
        Hashtable props = new Hashtable { { CustomRoomProperties.GameStarted, true } };
        PhotonNetwork.CurrentRoom.SetCustomProperties(props);

        // 2. A chamada para GameStartLogic() foi REMOVIDA DAQUI.
    }
