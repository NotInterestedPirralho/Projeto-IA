...
public class PlayerSetup : MonoBehaviourPunCallbacks, IPunObservable
{
    ...
    void Update()
    {
        ...
        if (!photonView.IsMine && anim)
        {
            ...
            // Sincroniza o parâmetro "IsRunning" (baseado na velocidade horizontal)
            anim.SetBool("IsRunning", Mathf.Abs(syncSpeed) > 0.1f);
            
            // Sincroniza o parâmetro "IsGrounded" (ou o que usa o seu Movement2D)
            anim.SetBool("IsGrounded", syncGrounded);
            
            // Nota: Se a animação de salto/queda usar "VerticalSpeed", precisa de o sincronizar também.
        }
    }
    ...
    public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
    {
        if (stream.IsWriting)
        {
            ...
            // Enviar velocidade horizontal e estado no chão (USANDO AS NOVAS PROPRIEDADES)
